...
class AddressNgramFeature(BaseFeatureExtraction):
    def __init__(self,min_df=10):
        super(AddressNgramFeature,self).__init__()
        self.method_info = "Street feature: using ngram"
        self.min_df = min_df
    def normalize(self,df_):
        df = df_.copy()
        addr_mapping={r"&":' and ','second':'2nd','first':'1st','third':'3rd','fourth':'4th','fifth':'5th',
                       'sixth':'6th','seventh':'7th','eighth':'8th','ninth':'9th','tenth':'10th'}

        for k,v in addr_mapping.iteritems():
            df['display_address'] = df['display_address'].map(lambda x:re.sub(k,v,x))
            df['display_address'] = df['display_address'].map(lambda x: x.lower())

        df['display_address'] = df['display_address'].map(lambda x:re.sub('[^0-9a-z\s]',' ',x))

        #df['masked_address'] = df['display_address'].map(lambda x:re.sub('([a-z ])?\d+[a-z]*',r"\1 xx",' '+x+' '))

        df['norm_address'] = df['display_address'].map(lambda x:re.sub('\sw\s ',' west ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('\se\s',' east ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('\sn\s',' north ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('\ss\s',' south ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('\sst\s',' street ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('\srd\s',' road ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('\save\s',' avenue ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('\sblvd\s',' boulevard ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('\spl\s',' place ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('\spkwy\s',' parkway ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('\sdr\s',' drive ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('\sln\s',' lane ',' '+x+' '))
        df['norm_address'] = df['norm_address'].map(lambda x:re.sub('(?<=[0-9])(?:st|nd|rd|th)','',x))
        return df
    
    def combine(self,X_counts,count_vect):
        all_keys = [x[0] for x in sorted(count_vect.vocabulary_.items(),key=lambda x:x[1])]

        addr_ngram = pd.DataFrame(X_counts.todense(),columns=all_keys)
        xxx_street = [x for x in all_keys if re.match('\d+ street',x) is not None]
        num_street = [int(re.sub('[^0-9]','',x)) for x in xxx_street]
        west_xxx = [x for x in all_keys if re.match('west \d+',x) is not None]
        west_num = [int(re.sub('[^0-9]','',x)) for x in west_xxx]
        east_xxx = [x for x in all_keys if re.match('east \d+',x) is not None]
        east_num = [int(re.sub('[^0-9]','',x)) for x in east_xxx]

        addr_ngram['xxx_street']=0
        addr_ngram['num_street']=0
        addr_ngram['west_xxx'] = 0
        addr_ngram['west_num'] = 0
        addr_ngram['east_xxx'] = 0
        addr_ngram['east_num'] = 0

        for a,b in zip(xxx_street,num_street):
            addr_ngram['xxx_street'] = addr_ngram['xxx_street'] + addr_ngram[a]
            addr_ngram.loc[addr_ngram[a]>0,'num_street'] = b
            del addr_ngram[a]

        for a,b in zip(west_xxx,west_num):
            addr_ngram['west_xxx'] = addr_ngram['west_xxx'] + addr_ngram[a]
            addr_ngram.loc[addr_ngram[a]>0,'west_num'] = b
            del addr_ngram[a]

        for a,b in zip(east_xxx,east_num):
            addr_ngram['east_xxx'] = addr_ngram['east_xxx'] + addr_ngram[a]
            addr_ngram.loc[addr_ngram[a]>0,'east_num'] = b
            del addr_ngram[a]
        return addr_ngram
    ...
    def transform(self,train_df,test_df):
        ...
        count_vect = CountVectorizer(ngram_range=(2,2),stop_words='english',min_df=self.min_df)
        count_vect.fit(train_df.norm_address)
        X_train_counts = count_vect.transform(train_df.norm_address)
        X_test_counts = count_vect.transform(test_df.norm_address)
        addr_ngram_tr = self.combine(X_train_counts,count_vect)
        addr_ngram_te = self.combine(X_test_counts,count_vect)
        
        fealist = list(addr_ngram_tr.columns)
        train_df = train_df.join(addr_ngram_tr)
        test_df = test_df.join(addr_ngram_te)
